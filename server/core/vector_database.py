...
import langchain_core.language_models as _language_models

if not hasattr(_language_models, "LangSmithParams"):
  # Provide minimal stub so older langchain-core builds work with google-genai package.
  class LangSmithParams(TypedDict, total=False):
    ls_provider: str
    ls_model_name: str
    ls_max_tokens: int

  _language_models.LangSmithParams = LangSmithParams

if not hasattr(_language_models, "is_openai_data_block"):
  def is_openai_data_block(_: object) -> bool:
    return False

  _language_models.is_openai_data_block = is_openai_data_block
